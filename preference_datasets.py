...
def get_se(split, silent=False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    return data
def get_pku(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the PKU-SafeRLHF dataset from Huggingface and convert it to the necessary format.
       ...
       For this dataset, the sft_target is the chosen response.
    """
    ...
    for row in tqdm.tqdm(dataset, desc='Processing PKU-SafeRLHF', disable=silent):
        prompt = '\n\nHuman: ' + row['prompt'] + '\n\nAssistant:'
        chosen = ' ' + row['response_0']
        rejected = ' ' + row['response_1']
        
        # Determine which response is better based on the better_response field
...
def clean_response(resp: str):
    resp = resp.strip()
    # remove any embedded </s> anywhere, not just at the end
...
def get_shp(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    for row in tqdm.tqdm(dataset, desc='Processing SHP', disable=silent):
        prompt = '\n\nHuman: ' + row['history'] + '\n\nAssistant:'
        respA = clean_response(row['human_ref_A'])
        respB = clean_response(row['human_ref_B'])
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name == 'shp':
        ...
        data = get_pku(split, silent=silent, cache_dir=cache_dir)
    elif name == 'se':
